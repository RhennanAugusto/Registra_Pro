...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="TRANSCRIÇÃO DAS TRANSMISSÕES (LIVRO 03)", font=("Helvetica", 15))
        label.pack(pady=10)
        ...
        button1 = tk.Button(center_frame, text="Criar meta", font=("Helvetica", 16),
                            command=lambda: controller.show_frame("CadastroDeMetasTranscricao"))
        button1.pack(pady=10)

        button2 = tk.Button(center_frame, text="Cadastrar relatorio", font=("Helvetica", 16),
                            command=lambda: controller.show_frame("PageProducaoDetalhes"))
        button2.pack(pady=10)

        button2 = tk.Button(center_frame, text="Historico de Cadastro", font=("Helvetica", 16),
                            command=lambda: controller.show_frame("PageHistorico"))
        button2.pack(pady=10)
